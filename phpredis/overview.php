...
<?php foreach ($config['servers'] as $i => $server) { ?>
  ...
  <?php if(!$info[$i]): ?>
  <div style="text-align:center;color:red">服务器</div>
  <?php else: ?>

  <table>

  <tr><td><div>Redis版本:</div></td><td><div><?php echo $info[$i]['Server']['redis_version']?></div></td></tr>

  <tr><td><div>Keys:</div></td><td><div><?php echo $info[$i]['size']?></div></td></tr>

  <tr><td><div>使用内存:</div></td><td><div><?php echo format_size($info[$i]['Memory']['used_memory'])?></div></td></tr>

  <tr><td><div>更新时间:</div></td><td><div><?php echo format_time($info[$i]['Server']['uptime_in_seconds'])?></div></td></tr>

  <tr><td><div>最后保存时间:</div></td><td><div>
    <?php
...
<p class="clear">
<a href="https://github.com/ErikDubbelboer/phpRedisAdmin" target="_blank">GitHub官方</a>
</p>
<?php
