...
if ($databases > 1) { ?>
  ...
  <option value="<?php echo $d?>" <?php echo ($server['db'] == $d) ? 'selected="selected"' : ''?>>db<?php echo $d?></option>
  <?php } ?>
  </select>
<?php } ?>
</p>
...
<p>
<a href="?edit&amp;s=<?php echo $server['id']?>&amp;d=<?php echo $server['db']?>" class="add">添加一个key</a>
</p>
...
<button id="btn_server_filter">刷选</button>
</p>

<p>
<input type="text" id="filter" size="24" value="type here to filter" placeholder="type here to filter" class="info">
</p>
<button id="selected_all_keys">全选</button>
<button id="operations">
<a href="delete.php?s=<?php echo $server['id']?>&amp;d=<?php echo $server['db']?>&batch_del=1" class="batch_del">删除选择<img src="images/delete.png" style="width: 1em;height: 1em;vertical-align: middle;" title="删除全选" alt="[X]"></a>
</button>
<div id="keys">
<ul>
...
<div style="color:red">不能连接到到这个服务器</div>
<?php } ?>
